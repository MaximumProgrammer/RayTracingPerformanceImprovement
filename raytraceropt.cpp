...
//https://stackoverflow.com/questions/18662261/fastest-implementation-of-sine-cosine-and-square-root-in-c-doesnt-need-to-b/18662397#18662397
//https://stackoverflow.com/questions/11261170/c-and-maths-fast-approximation-of-a-trigonometric-function

static float inline invSqrtFast(const float number ){
    union {
        float f;
        uint32_t i;
    } conv;
...
static float inline sqrtFast(const float x)
{
...
public:
    ...
    Vec3& normalize()
    {
        T nor2 = length2();
        if (nor2 > 0) {
            T invNor = invSqrtFast(static_cast<float>(nor2));
...
static float inline mix(const float a, const float b, const float mix)
{
    ...
    const int &depth)
{
    ...
    if ((sphere->transparency > 0 || sphere->reflection > 0) && depth < MAX_RAY_DEPTH) {
        ...
        if (sphere->transparency) {
            ...
            Vec3f refrdir = raydir * eta + nhit * (eta *  cosi - sqrtFast(k));
            refrdir.normalize();
...
//[/comment]
decltype(auto) render(const std::vector<Sphere> &spheres, const unsigned width = 4*640,  const unsigned height = 4* 480, const bool saveToFile=false)
{     
    ...
    const auto computeRay = [&] (const auto y){
	    ...
	    for (unsigned x = 0; x < width; ++x) { 
			...
			Vec3f raydir(xx, yy, -1);
                	...
                	pixel[x+y*width] = trace(Vec3f(0), raydir, spheres, 0);
                    }
   	       };
    ...
    for (unsigned y = 0; y < height; ++y) {         
	    threads.push_back(std::async(std::launch::async, computeRay, y));
    }
